...
if (isset($_POST['cetak'])) {
        ...
        if ($querysuplier) {
            foreach ($querysuplier as $suplier) {
                if (isset($_POST['suplier_' . $suplier['id_barang_suplier']])) {
                    ...
                    if ($querywhere[1]['judul'] == "") $querywhere[1]['judul'] = "Suplier Barang: " . $suplier['barang_suplier_nama'];
    ...
    if (!isset($_POST['tanggal_all'])) {
        ...
        $querywhere[3]['val'] = "`tb_barang_data`.`barang_data_tanggal` BETWEEN '$dari' AND '$sampai'";
    }
}
